...
class Call(PyTgCalls):
        ...
        try:
            ...
            try:
                if await self.is_in_group_call(assistant, chat_id):
                    ...
                    await _clear_(chat_id)
                    await app.send_message(original_chat_id, text="Bot state mismatch. Please use /reboot and try again.")
                    return
            except GroupCallNotFound:
                LOGGER(__name__).error(f"Group call not found when changing stream in chat {chat_id}")
                await _clear_(chat_id)
                await app.send_message(original_chat_id, text="Bot is not in a voice chat. Please use /reboot and try again.")
                return
            except Exception as e:
                LOGGER(__name__).error(f"Error changing stream in chat {chat_id}: {str(e)}")
                await _clear_(chat_id)
                await app.send_message(original_chat_id, text="Failed to change stream. Please use /reboot and try again.")
                return
        except TelegramServerError:
            LOGGER(__name__).error(f"Telegram server error in chat {chat_id}")
            await app.send_message(original_chat_id, text=_["call_10"])
            ...
            if "phone.CreateGroupCall" in str(e):
                await app.send_message(original_chat_id, text=_["call_8"])
                raise AssistantErr(_["call_8"])
            await app.send_message(original_chat_id, text="Failed to join voice chat due to an unexpected error.")
    ...
    async def decorators(self):
        ...
        async def stream_services_handler(_, chat_id: int):
            await self.stop_stream(chat_id)
            await app.send_message(chat_id, text="Bot was kicked or VC closed. Please use /reboot and try again.")
